...
*/
    ...
    /*
- -------------------------------CONSTANTS
Those values ??that, once the program has been compiled, cannot be changed, such as the literal values ??that we have used so far in the initializations of the variables (1000, 2, 'a', '\ n', etc.), are usually referred to as CONSTANTS
As these constants are stored in memory in the way that is most efficient for the compiler, certain side effects, sometimes disconcerting, usually appear, since they are affected by the rules of AUTOMATIC RECONVERSION OF TYPE previously seen.
In order to have control over the type of constants, the following rules apply:

A variable expressed as an integer (without decimal part) is taken as such unless it is followed by the letters F or L (upper or lower case) examples:
1: taken as WHOLE
1F: taken as FLOAT
1L: taken as LONG DOUBLE
A variable with decimal part is always taken as DOUBLE, unless it is followed by the letter F or L
1.0: taken as DOUBLE
1.0F: taken as FLOAT
1.0L: taken as LONG FLOAT
If in any of the above cases we add the letter U or u the constant is qualified as UNSIGNED (getting greater scope):
1u: taken as UNSIGNED INT
1.0UL: taken as UNSIGNED LONG DOUBLE
A numerical variable that begins with "0" is taken as OCTAL like this: 012 equals 10 decimal units
A numerical variable that begins with "0x" or "0X" is taken as hexadecimal: 0x16 is equivalent to 22 decimal units and 0x1A to 26 decimal units.
--------------------------------------SYMBOLIC CONSTANTS
It is usually a bad programming practice to place in a constant program in a literal form (especially if they are used several times in it) since the text becomes difficult to understand and even more to correct, if the value must be changed of these constants.
You can instead assign a symbol to each constant, and replace it throughout the program with it, so that it is more readable and also, if you want to change the value, just change it in the assignment.
The compiler, at the time of creating the executable, will replace the symbol with the assigned value.
To give a symbol to a constant it will be enough, anywhere in the program (prior to its use) to put the directive: #define, for example:


*/
#define VALUE_CONSTANTE 342
#define PI 3.1416
/*
...
-#######################################TYPES OF ERRoRS  IN COMPUTERS PROGRAMS:###########################
/*----------------------------------------------------------- Execution  --------------------           
